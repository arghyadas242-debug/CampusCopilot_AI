...
async function fetchAcademicSummary(studentRoll) {
  ...
  const response = await fetch(
    `${API_URL}/api/students/${encodeURIComponent(studentRoll)}/academic-summary`,
    {
...
function findExactStudent(students, query) {
  ...
  const exact = students.find(
    (student) =>
      String(student.STUDENT_ROLL || "").trim().toLowerCase() === normalizedQuery
  );
...
function getStudentManagementPath(studentRoll) {
  if (!studentRoll) return "/admin/students";
  return `/admin/students?student=${encodeURIComponent(studentRoll)}`;
}
...
export default function UpdateStudent() {
  ...
  const selectedStudentRoll = String(searchParams.get("student") || "").trim();

  // ---------------------------------------------------
  // SEARCH STATE (Independent from loaded student)
  // ---------------------------------------------------
  ...
  const [studentData, setStudentData] = useState(createEmptyStudent);
  ...
  // ---------------------------------------------------
  const [academicSummary, setAcademicSummary] = useState(createEmptyAcademicSummary);
  const [academicSummaryExists, setAcademicSummaryExists] = useState(false);
  const [academicSummaryLoading, setAcademicSummaryLoading] = useState(false);
  const [academicSummarySaving, setAcademicSummarySaving] = useState(false);
  const [academicSummaryError, setAcademicSummaryError] = useState("");
  ...
  function resetAcademicSummary() {
    setAcademicSummary(createEmptyAcademicSummary());
  ...
  // (Notice: does NOT touch searchInput)
  // ---------------------------------------------------
  function applyStudent(student) {
    const roll = String(student?.STUDENT_ROLL || "").trim();
    setStudentData({
  ...
  function applyAcademicSummary(data) {
    setAcademicSummary({
      cgpa:
        data?.cgpa === null || data?.cgpa === undefined ? "" : String(data.cgpa),
      creditsEarned:
        data?.creditsEarned === null || data?.creditsEarned === undefined
          ? ""
    ...
    setAcademicSummaryExists(Boolean(data?.hasAcademicSummary));
  }

  async function loadAcademicSummary(studentRoll) {
    if (!studentRoll) {
    ...
    try {
      ...
      setAcademicSummaryError("");
      const data = await fetchAcademicSummary(studentRoll);
      applyAcademicSummary(data);
    } catch (err) {
      console.error("Academic summary load error:", err);
      resetAcademicSummary();
      setAcademicSummaryError(
        err.message || "Unable to load academic summary."
      );
    } finally {
  ...
  // Depends strictly on selectedStudentRoll
  // ---------------------------------------------------
  useEffect(() => {
    ...
    async function loadUrlStudent() {
      try {
        setSearching(true);
        setError("");
        const results = await fetchStudentSearchResults(selectedStudentRoll);
        if (cancelled) return;
        ...
        const student = findExactStudent(results, selectedStudentRoll);
        if (!student) {
        ...
        setAcademicSummaryLoading(true);
        try {
          const summary = await fetchAcademicSummary(roll);
          if (!cancelled) {
            applyAcademicSummary(summary);
          }
        } catch (summaryError) {
          if (!cancelled) {
            console.error("Academic summary load error:", summaryError);
            resetAcademicSummary();
            setAcademicSummaryError(
              summaryError.message || "Unable to load academic summary."
            );
          }
        } finally {
          if (!cancelled) {
        ...
        if (!cancelled) {
          console.error("URL student load error:", err);
  ...
  async function handleSearch(event) {
    ...
    try {
      ...
      const results = await fetchStudentSearchResults(query);
      if (results.length === 0) {
      ...
      const exactStudent = findExactStudent(results, query);
      if (exactStudent) {
        await selectStudent(exactStudent);
      } else {
        setStudentLoaded(false);
        resetAcademicSummary();
        setSearchResults(results);
      ...
      setError(err.message || "Unable to search student.");
    } finally {
      setSearching(false);
    }
  }
  ...
  function handleAcademicSummaryChange(event) {
    const { name, value } = event.target;
    setAcademicSummary((previous) => ({
      ...previous,
      [name]: value,
    }));
    setAcademicSummaryError("");
  ...
  async function handleSaveStudent(event) {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/students/${encodeURIComponent(studentData.rollNumber)}`,
        {
          method: "PUT",
  ...
  // ---------------------------------------------------
  async function handleSaveAcademicSummary(event) {
    event.preventDefault();
    if (!studentData.rollNumber) {
      setError("No student selected.");
      return;
    ...
    const { cgpa, creditsEarned, totalProgramCredits, completedSemesters } =
      academicSummary;
    ...
    if (
      ...
      setError("Enter all four academic summary values.");
      return;
    }

    const parsedCgpa = Number(cgpa);
    const parsedCreditsEarned = Number(creditsEarned);
    const parsedTotalProgramCredits = Number(totalProgramCredits);
    const parsedCompletedSemesters = Number(completedSemesters);
    ...
    if (!Number.isInteger(parsedCreditsEarned) || parsedCreditsEarned < 0) {
      setError("Credits earned must be a whole number of 0 or more.");
      return;
    }

    if (!Number.isInteger(parsedTotalProgramCredits) || parsedTotalProgramCredits < 0) {
      setError("Total program credits must be a whole number of 0 or more.");
      return;
    }

    if (parsedCreditsEarned > parsedTotalProgramCredits) {
      setError("Credits earned cannot exceed total program credits.");
      return;
    }

    if (
      !Number.isInteger(parsedCompletedSemesters) ||
      parsedCompletedSemesters < 0 ||
      parsedCompletedSemesters > 8
    ) {
      setError("Completed semesters must be a whole number between 0 and 8.");
      return;
    }

    const currentSemester = Number(studentData.semester);
    if (
      Number.isInteger(currentSemester) &&
      currentSemester >= 1 &&
      parsedCompletedSemesters >= currentSemester
    ) {
      setError(
    ...
    const token = getToken();
    if (!token) {
      setError("Admin login token was not found. Please log in again.");
      return;
    }

    try {
      setAcademicSummarySaving(true);
      clearMessages();
      setAcademicSummaryError("");
      ...
      const response = await fetch(
        ...
        )}/academic-summary`,
        {
          ...
          body: JSON.stringify({
            cgpa: parsedCgpa,
            creditsEarned: parsedCreditsEarned,
            totalProgramCredits: parsedTotalProgramCredits,
            completedSemesters: parsedCompletedSemesters,
          }),
        }
      );

      await readJson(response);
      await loadAcademicSummary(studentData.rollNumber);
      setSuccess("Official academic summary saved successfully.");
    } catch (err) {
      console.error("Academic summary save error:", err);
      setError(err.message || "Unable to save academic summary.");
    } finally {
      setAcademicSummarySaving(false);
    }
  }
  ...
  async function handleDeleteStudent() {
    ...
    try {
      ...
      await readJson(response);
      setStudentData(createEmptyStudent());
      setStudentLoaded(false);
      setSearchResults([]);
      resetAcademicSummary();
      setMode("search");
  ...
  async function handleAddStudent(event) {
    ...
    try {
      ...
      const createdStudent = {
        ...
        NAME: newStudent.fullName,
        EMAIL: newStudent.email,
        STUDENT_ROLL: newStudent.rollNumber,
        DEPARTMENT: newStudent.department,
        SEMESTER: newStudent.semester,
        SECTION: newStudent.section,
      };
      ...
      await loadStudents();
      await selectStudent(createdStudent);
  ...
  function getInitials() {
    const name = String(studentData.fullName || "").trim();
    if (!name) return "?";
    return name
      .split(/\s+/)
      .filter(Boolean)
    ...
    <div className="min-h-screen bg-surface text-on-surface font-body-md antialiased">
      ...
      <main className="min-h-screen md:ml-[280px] min-w-0">
        ...
        <div className="w-full max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8 py-6 space-y-5">
          ...
          {/* Feedback Banners */}
          {error && <MessageBanner type="error" icon="error" text={error} />}
          {success && (
            <MessageBanner type="success" icon="check_circle" text={success} />
          )}
          ...
          {mode !== "add" && (
            <>
              ...
              {searchResults.length > 0 && (
                <section className="bg-surface-container-lowest rounded-2xl border border-outline-variant/70 overflow-hidden shadow-sm">
                  {searchResults.map((student) => (
                    <button
                      key={student.STUDENT_ID || student.STUDENT_ROLL}
                      type="button"
              ...
              {/* Empty Search Prompt */}
              {!studentLoaded && searchResults.length === 0 && (
                <section className="bg-surface-container-lowest border border-outline-variant/70 rounded-2xl p-10 text-center shadow-sm">
                  ...
                  <h2 className="font-bold text-lg mt-3">Search for a student</h2>
              ...
              {/* Selected Student Management Section */}
              {studentLoaded && (
                <>
                  ...
                  {/* Row 2: Official Academic Summary (Full-width underneath profile & edit cards) */}
                  <section className="w-full bg-surface-container-lowest rounded-2xl border border-outline-variant/70 p-5 md:p-6 shadow-sm">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                      ...
                      >
                        {academicSummaryExists ? "RECORDED" : "NOT RECORDED"}
                      </span>
                    </div>
                    ...
                    {academicSummaryLoading ? (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              value={academicSummary.cgpa ?? ""}
                              onChange={handleAcademicSummaryChange}
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              value={academicSummary.completedSemesters ?? ""}
                              onChange={handleAcademicSummaryChange}
                              className="w-full p-2.5 rounded-xl border border-outline-variant bg-surface-container-low text-sm font-semibold text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                              required
                            />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              value={academicSummary.creditsEarned ?? ""}
                              onChange={handleAcademicSummaryChange}
                              className="w-full p-2.5 rounded-xl border border-outline-variant bg-surface-container-low text-sm font-semibold text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                              required
                            />
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              value={academicSummary.totalProgramCredits ?? ""}
                              onChange={handleAcademicSummaryChange}
                              className="w-full p-2.5 rounded-xl border border-outline-variant bg-surface-container-low text-sm font-semibold text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                              required
                            />
                        ...
                        <div className="flex justify-end pt-1">
                          <button
                            type="submit"
                            disabled={academicSummarySaving || deleting}
...
function InputField({ label, helpText, className = "", value, ...props }) {
  return (
    <div className="flex flex-col gap-1">
      {label && (
        <label className="font-label-caps text-outline text-xs uppercase">
      ...
      />
      {helpText && (
        <span className="text-[10px] text-outline">{helpText}</span>
      )}
    </div>
  );
}
...
function MiniStat({ label, value }) {
  return (
    <div className="rounded-xl bg-surface-container-low border border-outline-variant/50 px-3 py-2">
      <p className="text-[10px] text-outline uppercase">{label}</p>
      <p className="font-bold text-primary mt-1">{value}</p>
    </div>
  );
}

function MessageBanner({ type, icon, text }) {
  const style =
    type === "error"
...
function StudentDirectory({ students, loading, error, onRetry }) {
  return (
    <section className="w-full bg-surface-container-lowest border border-outline-variant/70 rounded-2xl shadow-sm overflow-hidden">
      ...
      {loading ? (
        <div className="p-10 text-center text-on-surface-variant">
          ...
          <p className="font-body-sm text-sm mt-2">Loading students...</p>
        </div>
      ) : error ? (
          ...
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/70">
              {students.map((student, index) => (
                <tr
                  key={student.STUDENT_ID || student.STUDENT_ROLL || index}
                ...
                >
                  ...
                  <td className="py-3 px-3 text-right">
                    <Link
                      to={getStudentManagementPath(student.STUDENT_ROLL)}
