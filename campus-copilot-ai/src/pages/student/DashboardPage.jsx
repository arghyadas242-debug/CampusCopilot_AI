...
export default function DashboardPage() {
  ...
  const department =
    currentUser?.department || "Department not available";
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    attendanceStatus: "Attendance unavailable",
    ...
    recentNotices: [],
    upcomingAssignments: [],
    attendanceAvailable: rollNumber ? null : false,
    assignmentsAvailable: rollNumber ? null : false,
    examsAvailable: rollNumber ? null : false,
    timetableAvailable: rollNumber ? null : false,
    noticesAvailable: rollNumber ? null : false,
  });
    ...
    async function fetchRealData() {
      ...
      try {
        ...
        if (Array.isArray(rawAssignments)) {
          pendingAssignments = rawAssignments.filter(
            (assignment) => {
              const status = String(
                assignment.status ||
                  assignment.STATUS ||
        ...
        const rawExams = Array.isArray(exams)
          ? exams
          : exams?.exams;

        const upcomingExamsCount = Array.isArray(rawExams)
          ? rawExams.length
          : 0;
        ...
        const rawTimetable = Array.isArray(timetable)
          ? timetable
          : timetable?.classes;
        ...
        if (Array.isArray(rawTimetable)) {
          ...
          const matches = rawTimetable.filter((item) => {
            const day = String(
              item.day_of_week ||
                item.DAY_OF_WEEK ||
                ""
          ...
          todayClasses = matches.map((item, index) => ({
            subject:
              item.subject_name ||
              item.SUBJECT_NAME ||
              item.subject_code ||
        ...
        const rawNotices = Array.isArray(notices)
          ? notices
          : notices?.notices;

        const recentNotices = Array.isArray(rawNotices)
          ? rawNotices.slice(0, 2).map((notice, index) => ({
              id:
                notice.id ||
                notice.ID ||
                index + 1,
        ...
        setDashboardData((prev) => ({
          ...
          examsAvailable: exams !== null,
          timetableAvailable: timetable !== null,
          noticesAvailable: notices !== null,
        }));
      } catch (error) {
        console.warn(
    ...
    <div className="bg-background text-on-background font-body-md antialiased min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col md:flex-row w-full">
        ...
        <main className="flex-1 min-w-0 p-margin-mobile md:p-md lg:p-margin-desktop overflow-y-auto pb-[100px] md:pb-lg">
          {/* WELCOME BANNER */}
          <section className="hidden md:block relative z-40 overflow-visible bg-gradient-to-r from-primary to-primary-container rounded-2xl px-lg py-md mb-md">
            {/* Only the decorations are clipped. */}
            <div
              aria-hidden="true"
            ...
            {/* Content and notification dropdown remain unclipped. */}
                ...
                <div className="divide-y divide-surface-container-high">
                  {dashboardData.recentNotices.map((notice) => (
                    ...
                    >
                      ...
                      >
                        <span className="material-symbols-outlined text-[18px]">
                          {notice.isUrgent ? "warning" : "campaign"}
                        </span>
                      </div>
