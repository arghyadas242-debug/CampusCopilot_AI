...
async function readResponse(response, failureMessage) {
  if (response.status === 401) {
    const error = new Error(
      "Please sign in again to access notifications."
    );
  ...
  if (response.status === 403) {
    const error = new Error(
      "Access denied. You may not have permission, or your session may have expired."
    );
    error.status = response.status;
    throw error;
  }
...
export default function StudentNotificationBell() {
  ...
  // LOGGED-IN STUDENT
  //
  // This roll selects the requested resource.
  // The backend independently verifies ownership using JWT.
  // =====================================================
  ...
  const loadNotifications = useCallback(async () => {
    if (!studentRoll) {
      setNotifications([]);
      setLoading(false);
      setError("Student information unavailable");
      return;
    }
    ...
    try {
      ...
      const normalized = rawNotifications.map(
  ...
  const handleNotificationClick = async (notification) => {
    if (!notification.isRead) {
      ...
      // Keep the dropdown open so a failed update is visible.
      if (!updated) {
        return;
      }
    }
    ...
    >
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[min(92vw,390px)] bg-surface-container-lowest border border-outline-variant/70 rounded-2xl shadow-2xl overflow-hidden z-[100]">
          ...
          {/* ERROR — ALSO VISIBLE WHEN EXISTING ITEMS REMAIN */}
          {!loading && error && (
            <div
              role="alert"
